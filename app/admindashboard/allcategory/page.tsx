...
export default function AdminAllCategoryPage() {
  ...
  const handleDeleteCategory = async (index: number) => {
    ...
    try {
      await deleteDoc(doc(db, "allcategories", categories[index].docId!));
      fetchCategories();
    } catch (err) {
      alert("Delete failed.");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editIndex !== null && categories[editIndex]?.docId) {
        await updateDoc(
          doc(db, "allcategories", categories[editIndex].docId!),
          {
            ...data,
        ...
        await addDoc(collection(db, "allcategories"), data);
  ...
  <nav className="space-y-2">
    ...
    <a
      ...
      className="flex items-center space-x-3 px-4 py-3 rounded-lg bg-slate-700"
    >
      <svg
              className="w-5 h-5"
              fill="none"
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-gray-800">
              {editIndex !== null ? "Edit Category" : "Add All Products"}
            </h2>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              >
                ...
                >
                  {editIndex !== null ? "Update Category" : "Add All Products"}
                </button>
              </form>
            </div>
          ...
          <section className="mt-12">
            ...
            {categories.length === 0 ? (
              <p className="text-gray-500">No All Products yet.</p>
            ) : (
